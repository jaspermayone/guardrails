#!/usr/bin/env bash
#
# Remote installer for guardrails PreToolUse hook
...
REPO_BASE="https://raw.githubusercontent.com/jaspermayone/guardrails/main"
CLAUDE_DIR="${HOME}/.claude"
HOOKS_DIR="${CLAUDE_DIR}/hooks"
HOOKS_FILE="${CLAUDE_DIR}/settings.json"
GUARDRAIL_DEST="${HOOKS_DIR}/guardrail.py"
POLICY_DEST="${HOOKS_DIR}/policy.yaml"

echo "Installing guardrails PreToolUse hook from remote..."
...
# Download guardrail.py
echo "Downloading guardrail.py..."
if ! curl -fsSL "${REPO_BASE}/guardrail.py" -o "$GUARDRAIL_DEST"; then
    echo "Error: Failed to download guardrail.py"
    exit 1
fi
chmod +x "$GUARDRAIL_DEST"

# Download policy.yaml
echo "Downloading policy.yaml..."
if ! curl -fsSL "${REPO_BASE}/policy.yaml" -o "$POLICY_DEST"; then
    echo "Error: Failed to download policy.yaml"
    rm -f "$GUARDRAIL_DEST"  # Clean up on failure
    exit 1
fi
...
if 'env' not in settings:
    settings['env'] = {}
settings['env']['GUARDRAILS_POLICY_PATH'] = "${POLICY_DEST}"
